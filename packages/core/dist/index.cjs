...
function equalsRefList(refListA, refListB) {
...
class Property extends propertyGraph.GraphNode {
  ...
  copy(other, resolve) {
    ...
    for (const key in this[propertyGraph.$attributes]) {
      ...
      } else if (propertyGraph.isRefList(value)) {
        for (const ref of value) {
          ref.dispose();
        }
      } else if (propertyGraph.isRefMap(value)) {
    ...
    for (const key in other[propertyGraph.$attributes]) {
      ...
      } else if (propertyGraph.isRefList(otherValue)) {
        for (const ref of otherValue) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          this.addRef(key, resolve(ref.getChild()), ref.getAttributes());
        }
      } else if (propertyGraph.isRefMap(otherValue)) {
  ...
  equals(other, skip) {
    ...
    for (const key in this[propertyGraph.$attributes]) {
      ...
      if (propertyGraph.isRef(a) || propertyGraph.isRef(b)) {
        if (!equalsRef(a, b)) {
          return false;
        }
      } else if (propertyGraph.isRefList(a) || propertyGraph.isRefList(b)) {
        if (!equalsRefList(a, b)) {
          return false;
        }
      } else if (propertyGraph.isRefMap(a) || propertyGraph.isRefMap(b)) {
        if (!equalsRefMap(a, b)) {
          return false;
        }
...
class Accessor extends ExtensibleProperty {
  ...
  setNormalized(normalized) {
    ...
    if (normalized) {
      this._out = i => MathUtils.decodeNormalizedInt(i, this.get('componentType'));
...
class Node extends ExtensibleProperty {
  ...
  addChild(child) {
    ...
    const childrenRefs = this[propertyGraph.$attributes]['children'];
...
class Document {
  ...
  merge(other) {
    ...
    for (const edge of other._graph.listEdges()) {
      for (const otherProp of [edge.getParent(), edge.getChild()]) {
        if (visited.has(otherProp)) continue;
        let thisProp;
        if (otherProp.propertyType === exports.PropertyType.TEXTURE_INFO) {
    ...
    for (const otherProp of visited) {
      ...
      if (thisProp.propertyType !== exports.PropertyType.TEXTURE_INFO) {
  ...
  transform() {
    try {
      ...
      const _temp = _forOf(transforms, function (transform) {
    ...
    try {
      ...
      return Promise.resolve(_this4.writeJSON(doc, {
        ...
        return Promise.resolve(fs.writeFile(uri, jsonContent)).then(function () {
          const pending = Object.keys(resources).map(function (resourceURI) {
            try {
              ...
              return Promise.resolve(fs.mkdir(path.dirname(resourcePath), {
                ...
                return Promise.resolve(fs.writeFile(resourcePath, resource)).then(function () {
                  _this4.lastWriteBytes += resource.byteLength;
                });
              });
            } catch (e) {
              return Promise.reject(e);
